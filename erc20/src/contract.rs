...
use crate::msg::{AllowanceResponse, BalanceResponse, ERC20HandleMsg, InitMsg, QueryMsg};
use cosmwasm_std::{
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    match msg {
        ERC20HandleMsg::Approve { spender, amount } => try_approve(deps, env, &spender, &amount),
        ERC20HandleMsg::Transfer { recipient, amount } => try_transfer(deps, env, &recipient, &amount),
        ERC20HandleMsg::TransferFrom {
            owner,
            recipient,
            amount,
        } => try_transfer_from(deps, env, &owner, &recipient, &amount),
        ERC20HandleMsg::Burn { amount } => try_burn(deps, env, &amount),
    }
}
