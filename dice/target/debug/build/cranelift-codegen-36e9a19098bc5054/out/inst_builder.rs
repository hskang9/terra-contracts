...
pub trait InstBuilder<'f>: InstBuilderBase<'f> {
    ...
    /// - a: A SIMD vector type
    #[allow(non_snake_case)]
    fn insertlane<T1: Into<ir::immediates::Uimm8>>(self, x: ir::Value, Idx: T1, y: ir::Value) -> Value {
        let Idx = Idx.into();
        let ctrl_typevar = self.data_flow_graph().value_type(x);
    ...
    fn x86_pextr<T1: Into<ir::immediates::Uimm8>>(self, x: ir::Value, Idx: T1) -> Value {
        let Idx = Idx.into();
        let ctrl_typevar = self.data_flow_graph().value_type(x);
        let (inst, dfg) = self.ExtractLane(Opcode::X86Pextr, ctrl_typevar, Idx, x);
        dfg.first_result(inst)
    }
    ...
    #[allow(non_snake_case)]
    fn x86_pinsr<T1: Into<ir::immediates::Uimm8>>(self, x: ir::Value, Idx: T1, y: ir::Value) -> Value {
        let Idx = Idx.into();
        let ctrl_typevar = self.data_flow_graph().value_type(x);
        let (inst, dfg) = self.InsertLane(Opcode::X86Pinsr, ctrl_typevar, Idx, x, y);
        dfg.first_result(inst)
    }
    ...
    #[allow(non_snake_case)]
    fn x86_insertps<T1: Into<ir::immediates::Uimm8>>(self, x: ir::Value, Idx: T1, y: ir::Value) -> Value {
        let Idx = Idx.into();
        let ctrl_typevar = self.data_flow_graph().value_type(x);
        let (inst, dfg) = self.InsertLane(Opcode::X86Insertps, ctrl_typevar, Idx, x, y);
        dfg.first_result(inst)
    }
